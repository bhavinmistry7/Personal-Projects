public class HeartTransplant {
    ...
    public int readSurvivabilityRateByAgeFromFile (int numberOfLines) {
        ...
        for (int i =0; i < numberOfLines; i++) {
            ...
            SurvivabilityByAge ageSurvivability = new SurvivabilityByAge(age, yearsPostTransplant, rate);
            survivabilityByAge[i] = ageSurvivability;
    ...
    public int readSurvivabilityRateByCauseFromFile (int numberOfLines) {
        ...
        for (int i = 0; i < numberOfLines; i++) {
            ...
            double rate = StdIn.readDouble();
            SurvivabilityByCause causeSurvivability = new SurvivabilityByCause(cause, years, rate);
            survivabilityByCause[i] = causeSurvivability; 
        }
    ...
    public Person[] getPatientsWithAgeAbove(int age) {
        ...
        int numsPatientsAboveAge = 0;
        for (int i = 0; i < listOfPatients.length; i++) {
            if (listOfPatients[i].getAge() > age) {

                numsPatientsAboveAge++;
            }
        }
        Person[] patientsssAboveAge = new Person[numsPatientsAboveAge];
        
        if (numsPatientsAboveAge == 0) {
            ...
            for (int i=0; i< listOfPatients.length; i++) {
                    ...
                    if (listOfPatients[i].getAge() > age) {
                        ...
                        patientsssAboveAge[index1] = listOfPatients[i];
                        index1++; 
                    }
                

            }
            return patientsssAboveAge;
        }
    }
    ...
    public Person[] getPatientsByStateOfHealth(int state) {
        ...
        Person[] pastientssAtState = new Person[numsPatientsAtState];

        if (numsPatientsAtState == 0) {
            ...
            for (int i=0; i< listOfPatients.length; i++) {
                    ...
                    if (listOfPatients[i].getStateOfHealth() == state) {
                        pastientssAtState[index2] = listOfPatients[i];
                        index2++;
                    }

            }
            return pastientssAtState;
        }
    }
    ...
    public Person[] getPatientsByHeartConditionCause(int cause) {
        ...
        int numsPatientsWithCause = 0;
        for (int i = 0; i < listOfPatients.length; i++) {
            if (listOfPatients[i].getCause() == cause) {

                numsPatientsWithCause++;
            }
        }
        Person[] patientssWithCause = new Person[numsPatientsWithCause];
        if (numsPatientsWithCause == 0) {
            return null;
        } else {
           ...
           int index3 = 0;
            for (int i=0; i< listOfPatients.length; i++) {
                    ...
                    if (listOfPatients[i].getCause() == cause) {
                        ...
                        patientssWithCause[index3] = listOfPatients[i];
                        index3++;
                    }

            }
            return patientssWithCause;
        }
    }
    ...
    public Person[] match(int numberOfHearts) {
        ...
        Person [] highestPotential = new Person[numberOfHearts];
        if (numberOfHearts <= listOfPatients.length) {

            for (int m = 0; m<numberOfHearts; m++) {

                highestPotential[m] = listOfPatients[m];

            }
        } else {

            Person temp = listOfPatients[0];
            for (int i = 0; i < listOfPatients.length; i++) {

                for (int j = i+ 1; j < listOfPatients.length; j++) {

                    if (listOfPatients[i].getUrgency() > listOfPatients[j].getUrgency()) {

                        temp = listOfPatients[i];
                        listOfPatients[i] = listOfPatients[j];
                        listOfPatients[j] = temp;


                        for (int k = 0; k < numberOfHearts; k++) {

                            highestPotential[k] = listOfPatients[k];
                        }
                    }
