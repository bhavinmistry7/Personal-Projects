...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int[] arr = new int[numberOfDaysInMonth[month - 2]];
        double dayOne = StdRandom.uniform();
        double dryWetPercentage = 0.0;
        double wetWetPercentage = 0.0;
        if (dayOne >= 0.5) {
            arr[0] = DRY;
        } else {
            arr[0] = WET;
        ...
        for (int i = 0; i < drywet.length; i++) {
            if (Double.compare(drywet[i][0], longitute) == 0 && Double.compare(drywet[i][1], latitude) == 0) {
                dryWetPercentage = drywet[i][month];
                break;
            }
        }

        for (int i = 0; i < wetwet.length; i++) {
            if (Double.compare(wetwet[i][0], longitute) == 0 && Double.compare(wetwet[i][1], latitude) == 0) {
                wetWetPercentage = wetwet[i][month];
                break;
            }
        }
        ...
        for (int i = 1; i < arr.length; i++) {
            if (arr[i - 1] == WET) {
                ...
                if (random <= wetWetPercentage) {
                    arr[i] = WET; 
                } else arr[i] = DRY;
            } else {
                double random2 = StdRandom.uniform();
                if (random2 <= dryWetPercentage) {
                    arr[i] = WET; 
                } else arr[i] = DRY;
            }
        }
